...
run_name = 'classical_unbiased'

file_name = 'Polaris/' + data_name + '.parquet'
matrix, matrix_labels, names = svm.read_parquet(file_name)

slice_sizes = [18,20] # [21,24,29,36,68] + [int(177/n) for n in [4,3,2,1]] + [203, 228, 305, 456, 609] # + [None]
limit = 12
# limit = 20
# limits = [34,26,18,12]
N = 20
...
for i in tqdm(range(start,N)):
    ...
    for s, slice_size in enumerate(slice_sizes):
        ...
        estimator = svm.slice_estimator(estimator=inner_estimator, slice_size=slice_size, force_unbiased=True, adjust_outer_bias=True, seed=0) # bool(data_name == 'bbb-martins')
        try:
